...
# PDBFixer is now a pre-processing step, so it is removed from this list
validators = {
...
def show_tool_outputs(tool_dir):
    ...
    # show logs
    for logfile in tool_dir.glob("*.log"):
        with st.expander(f"View {tool_dir.name} Logs"):
            st.code(logfile.read_text())
...
if run_validation_button:
    ...
    if pdb_file_initial:
        ...
        with st.spinner("Fixing PDB..."):
            fixed_pdb_path, pdbfixer_log_path = validation_utils.run_pdbfixer(pdb_for_processing, fixed_dir)

        if fixed_pdb_path:
            st.success("PDBFixer finished.")
            pdb_for_validation = fixed_pdb_path
            st.info(f"Using fixed PDB for subsequent validations: {pdb_for_validation}")
        else:
            st.error("PDBFixer failed. Subsequent validations will use the original PDB.")
            pdb_for_validation = pdb_for_processing  # Fallback to original
            if pdbfixer_log_path.exists():
                with st.expander("Show PDBFixer Error Log"):
                    st.code(pdbfixer_log_path.read_text())
        ...
        st.info("Starting validation pipeline...")
        for key, script_name in wrapper_order:
            ...
            if key == "voromqa":
                if not voromqa_path_input or not Path(voromqa_path_input).is_file():
                    ...
                    continue
                extra_args.extend(["--voromqa_path", voromqa_path_input])
            elif key == "qmean":
            ...
            with st.spinner(f"Running {key}..."):
                ...
                success, log_path = validation_utils.run_validation_tool(
                    tool_name=key,
            ...
            if success:
                ...
                show_tool_outputs(validation_dir / key)
            else:
                st.error(f"{key} failed. Check logs for details.")
                if log_path.exists():
                    with st.expander(f"Show {key} Error Log"):
                        st.code(log_path.read_text())
                show_tool_outputs(validation_dir / key) # Show other outputs even on failure
